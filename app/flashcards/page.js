      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-ink/60">
            Pick a topic and click any card to flip between the question and
            the answer.
          </p>
        </div>

        <div>
          <Link href="/flashcards?saved=true" className="btn-secondary">
            Saved Flashcards
          </Link>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        {learningTopics.map((topic) => {
          ...
          return (
            <Link
              key={topic}
              href={`/flashcards/${encodeURIComponent(slug)}`}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {(showSavedOnly
          ...
          return (
            <div
              key={card.id}
              className={`relative ${isFlipped ? "is-flipped" : ""}`}
            >
              <div
                role="button"
                tabIndex={0}
