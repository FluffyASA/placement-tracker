...
export default function StudyView() {
  ...
  const section = module && topicId ? findSection(moduleId, topicId) : null;

  if (!module || !section) {
    return (
      <div className="card">
        <p className="text-sm text-ink/60">Content not found. Go back to <Link href="/study">Study Materials</Link>.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/study" className="btn-secondary">Back</Link>
      </div>
